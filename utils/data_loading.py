...
'''
def non_iid_dirichlet_old(num_data, targets, alpha):
    ...
    label_idx = {}
    num_data_per_label = np.zeros(num_classes)
    start_idx = 0
    for c in range(num_classes):
        # determine amount of class c data
        num_c = torch.sum(targets == c)
        num_data_per_label[c] = num_c
        # index class c data
        c_data = sorted_indices[start_idx:start_idx + num_c]
        label_idx[c] = c_data
        start_idx += num_c

    # create dirichlet split of classes for device i (must be valid, i.e. it can't have more data than is available)
    proportions = np.ones(num_classes) * np.infty
    while np.any(proportions > num_data_per_label):
        proportions_unrounded = np.random.dirichlet(np.repeat(alpha, num_classes)) * num_data
        proportions = np.round(proportions_unrounded)
        proportions = proportions.astype(np.int32)
        nd = np.sum(proportions)
        if nd < num_data:
            proportions[np.argmax(proportions_unrounded - proportions)] += (num_data - nd)

    # fill the data for device i
    data_idx = np.empty(num_data, dtype=np.int32)
    count = 0
    for c, fill in enumerate(proportions):
        data_idx[count:count+fill] = np.random.choice(label_idx[c], fill)
        count += fill
